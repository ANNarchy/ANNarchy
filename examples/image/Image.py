...
smooth_proj = Convolution(pre=pooled, post=smoothed, target='exc', weights=box_filter, method='filter')


# To perform a convolution operation on the population (or more precisely a cross-correlation), we create a `ConvolutionProjection`. It requires to define a kernel (`weights`) that will be convolved over the input population. Here we use a simple box filter, but any filter can be used.
...
filter_proj = Convolution(pre=smoothed, post=filtered, target='exc', weights=filter_bank, method='filter', multiple=True)
