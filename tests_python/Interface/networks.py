...
class RateCodedNetwork(Network):
    def __init__(self):
        ...
        self.proj2 = self.connect(
            pre=self.pop1, post=self.pop3, target="exc", synapse=Hebb
        )
        self.proj2.all_to_all(1.0)

        self.proj3 = self.connect(
            pre=self.pop2, post=self.pop3, target="exc", synapse=Oja
        )
...
class SpikingNetwork(Network):
    def __init__(self):
        ...
        self.proj1 = self.connect(
            pre=self.pop1, post=self.pop2, target="exc", synapse=STDP
        )
        self.proj1.one_to_one(weights=Uniform(-0.5, 0.5))

        self.proj2 = self.connect(
            pre=self.pop2, post=self.pop3, target="exc", synapse=STP
        )
        self.proj2.all_to_all(1.0)
