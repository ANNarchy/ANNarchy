...
class test_IO_Rate(unittest.TestCase):
        ...
        for ext in self.save_extensions:
            with self.subTest(extension=ext):
                ...
                assert_allclose_named(
                    self.get_attributes(), self.new_attr, self.attribute_names
                )
    ...
    def test_parameters_save_and_load(self):
        ...
        assert_allclose_named(
            self.get_parameters(), self.init_attr, self.attribute_names, self.isparam
        )
...
class test_IO_Spiking(unittest.TestCase):
    ...
    def get_attributes(self):
        ...
        return [
            self.network.pop2.get("d")[0],
            self.network.pop2.get("v")[0],
            self.network.proj1.get("tau_plus"),
    ...
    def test_projection_save_and_load_connectivity(self):
        ...
        # single-weight
        self.network.proj0.save_connectivity(
            self.savefolder + "pr2spike_proj0_conn.npz"
        )
        # multi-weight
        self.network.proj1.save_connectivity(
            self.savefolder + "pr2spike_proj1_conn.npz"
        )
