...
class test_SpikingContinuousUpdate(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        """
        simple_pre_neuron = Neuron(equations="mp=t", spike="(modulo(t,3))")
        simple_post_neuron = Neuron(equations="mp=t", spike="(modulo(t,5))")
...
class test_ContinuousTransmission(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        SpkNeuron = Neuron(equations="v = Uniform(0,1)", spike="v > 0.9")

        # decrease weight until zero.
        ContSynapse = Synapse(psp="post.v - pre.v")
