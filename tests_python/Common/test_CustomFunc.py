...
class test_CustomFunc(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        neuron2 = Neuron(equations="r = glob_pos(sum(exc))")

        synapse = Synapse(
            equations="w += hebb(pre.r, post.r)", functions="hebb(x, y) = x * y"
        )
    ...
    def test_neuron(self):
        ...
        numpy.testing.assert_allclose(
            self._pop1.transfer_function(
                numpy.array([0.0, 1.0, 2.0, 3.0]), numpy.array([2.0, 2.0, 2.0, 2.0])
            ),
            [0, 0, 0, 1],
        )
    ...
    def test_neuron2(self):
        ...
        """
        numpy.testing.assert_allclose(
            self._pop1.transfer_function([0.0, 1.0, 2.0, 3.0], [2.0, 2.0, 2.0, 2.0]),
            [0, 0, 0, 1],
        )
    ...
    def test_synapse(self):
        ...
        numpy.testing.assert_allclose(
            self._proj.hebb(
                numpy.array([0.0, 1.0, 2.0, 3.0]), numpy.array([0.0, 1.0, 2.0, 3.0])
            ),
            [0, 1, 4, 9],
        )
    ...
    def test_synapse2(self):
        ...
        """
        numpy.testing.assert_allclose(
            self._proj.hebb([0.0, 1.0, 2.0, 3.0], [0.0, 1.0, 2.0, 3.0]), [0, 1, 4, 9]
        )
