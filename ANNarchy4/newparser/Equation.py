...
class Equation(object):
    ...
    #        expression = expression.replace('d'+name, 't_gradient_')
    #        expression = expression.replace(name, 'newval')
    #        expression = expression.replace('t_gradient_', '(newval - '+name+')')
    #
    #        # Add a sympbol for the next value of the variable
    #        newval = Symbol('newval')
    #        local_dict['newval'] = newval
    #
    #        # Parse the string
    ...
    #        explicit_equation = simplify(explicit_equation - local_dict[name])
    #
    #        # Obtain C code
    #        explicit_code = c_code(local_dict[name]) + ' += ' +  c_code(explicit_equation) + ';'
    #
    #        # Return result
    #        return explicit_code
    
        # TODO: be sure that only the linear functions of the variable are used to generate the expression !!! semi-implicit method only -> won't work for Izhikevitch or AEIF neurons 
    ...
    def exponential(self, expression, local_dict):
        # Standardize the equation
        real_tau, stepsize, steadystate = self.standardize_ODE(expression)
    
        # Update rule
        explicit_code = self.c_code(local_dict[self.name]) + ' += (1.0 - exp(' \
                        + self.c_code(simplify(-stepsize)) + ')))*(' \
                        + self.c_code(steadystate)+ ' - ' + self.c_code(local_dict[self.name]) +');'
    ...
    def standardize_ODE(self, expression):
        ...
        expression = expression.replace('d' + self.name +'/dt', 'gradvar') # TODO: robust to spaces
    
        # Add the gradient sympbol
        grad_var = Symbol('gradvar')
        ...
        collected_gradient = collect(expand(analysed, grad_var), grad_var, evaluate=False)
