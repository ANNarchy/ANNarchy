    ...
    def __init__(
        ...
        if description:
            ...
            self.short_description = (
                "User-defined model of a spiking neuron."
                if self.type == "spike"
                else "User-defined model of a rate-coded neuron."
            )
...
class IndividualNeuron:
    ...
    def __setattr__(self, name, value):
        ...
        if name == "population":
            ...
            if name in self.population.attributes:
                if name in self.population.neuron_type.description["local"]:
                    if (
                        not self.population.initialized
    ...
    def __repr__(self):
        desc = (
            "Neuron of the population "
            + self.population.name
            + " with rank "
            + str(self.rank)
            + " (coordinates "
            + str(self.population.coordinates_from_rank(self.rank))
            + ").\n"
        )
        desc += "Parameters:\n"
        ...
        if other.population == self.population:
            if isinstance(other, IndividualNeuron):
                return PopulationView(
                    self.population, list(set([self.rank, other.rank]))
                )
