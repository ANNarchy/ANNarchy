...
class Dendrite:
        ...
        if name == "proj":
            ...
            if name == "rank":  # TODO: remove 'rank' in a future version
                ...
                if self.proj.uniform_delay == -1:
                    return [
                        d * ConfigManager().get("dt", self.proj.net_id)
                        for d in self.proj.cyInstance.get_dendrite_delay(self.idx)
                    ]
                else:
                    return self.proj.max_delay * ConfigManager().get(
                        "dt", self.proj.net_id
                    )
                ...
                ctype = None
                for var in (
                    self.proj.synapse_type.description["variables"]
                    + self.proj.synapse_type.description["parameters"]
                ):
                    if var["name"] == name:
                        ctype = var["ctype"]
                ...
                if name in self.proj.synapse_type.description["local"]:
                    return getattr(
                        self.proj.cyInstance, "get_local_attribute_row_" + ctype
                    )(name, self.idx)
                elif name in self.proj.synapse_type.description["semiglobal"]:
                    return getattr(
                        self.proj.cyInstance, "get_semiglobal_attribute_" + ctype
                    )(name, self.idx)
                else:
                    return getattr(
                        self.proj.cyInstance, "get_global_attribute_" + ctype
                    )(name)
            else:
                return object.__getattribute__(self, name)
        else:
            ...
            if name in self.proj.attributes:
                ...
                if name in self.proj.synapse_type.description["local"]:
                    if isinstance(value, (np.ndarray, list)):
                        getattr(
                            self.proj.cyInstance, "set_local_attribute_row_" + ctype
                        )(name, self.idx, list(value))
                    else:
                        getattr(
                            self.proj.cyInstance, "set_local_attribute_row_" + ctype
                        )(name, self.idx, [value for _ in range(self.size)])

                elif name in self.proj.synapse_type.description["semiglobal"]:
                    getattr(self.proj.cyInstance, "set_semiglobal_attribute_" + ctype)(
                        name, self.idx, value
                    )
    ...
    def create_synapse(self, rank: int, w: float = 0.0, delay: float = 0) -> None:
        ...
        if self.proj.cyInstance.nb_dendrites() < self.proj.post.size:
            post_idx = np.where(
                np.array(self.proj.cyInstance.post_rank()) == self.post_rank
            )[0][0]
        else:
        ...
        for var in (
            ...
            if (
                ...
                and var["name"] in self.proj.synapse_type.description["local"]
            ):
    ...
    def create_synapses(
        ...
        delays = np.array(
            delays / ConfigManager().get("dt", self.proj.net_id), dtype=np.int32
        )
...
class IndividualSynapse:
    ...
    def __getattr__(self, name):
        ...
        if name in ["dendrite", "attributes", "rank"]:
            ...
            ctype = None
            for var in (
                self.dendrite.proj.synapse_type.description["variables"]
                + self.dendrite.proj.synapse_type.description["parameters"]
            ):
                if var["name"] == name:
                    ctype = var["ctype"]
            ...
            if name in self.dendrite.proj.synapse_type.description["local"]:
                return getattr(
                    self.dendrite.proj.cyInstance, "get_local_attribute_" + ctype
                )(name, self.dendrite.idx, self.rank)
            elif name in self.dendrite.proj.synapse_type.description["semiglobal"]:
                return getattr(
                    self.dendrite.proj.cyInstance, "get_semiglobal_attribute_" + ctype
                )(name, self.dendrite.idx)
            else:
                return getattr(
                    self.dendrite.proj.cyInstance, "get_global_attribute_" + ctype
                )(name)
    ...
    def __setattr__(self, name, value):
        ...
        if name in ["dendrite", "attributes", "rank"]:
            ...
            if name in self.dendrite.proj.synapse_type.description["local"]:
                getattr(self.dendrite.proj.cyInstance, "set_local_attribute_" + ctype)(
                    name, self.dendrite.idx, self.rank, value
                )
            elif name in self.dendrite.proj.synapse_type.description["semiglobal"]:
                getattr(
                    self.dendrite.proj.cyInstance, "set_semiglobal_attribute_" + ctype
                )(name, self.dendrite.idx, value)
            else:
                getattr(self.dendrite.proj.cyInstance, "set_global_attribute_" + ctype)(
                    name, value
                )
