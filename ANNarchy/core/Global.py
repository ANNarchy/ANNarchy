...
class every(object):
    ...
    The times at which the method is called are relative to the time when ``simulate()`` is called.
    ...
    def __init__(self, period, offset=0., wait=0.0):
        ...
        * **period**: interval in ms between two calls to the function.
        ...
        * **wait**: allows to wait for a certain amount of time (in ms) before starting to call the method. 
        ...
        ``wait`` can be combined with ``offset``, so if ``period=100.``, ``offset=50.`` and ``wait=500.``, the first call will be made 500 ms after the call to ``simulate()``
...
def simulate_with_callbacks(duration, net_id=0):
    """
    Tmp: replaces simulate when call_backs are defined.
    """
    if not _network[net_id]['instance']:
        _error('simulate(): the network is not compiled yet.')
        exit(0)
