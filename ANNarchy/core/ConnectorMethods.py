...
def connect_one_to_one(
    ...
    self.connector_weight_dist = (
        weights if isinstance(weights, RandomDistribution) else None
    )
    self.connector_delay_dist = (
        delays if isinstance(delays, RandomDistribution) else None
    )

    self._store_connectivity(
        one_to_one,
        (weights, delays, storage_format, storage_order),
        delays,
        storage_format,
        storage_order,
    )
...
def connect_all_to_all(
    ...
    """
    pre_pop = (
        self.pre if not isinstance(self.pre, PopulationView) else self.pre.population
    )
    post_pop = (
        self.post if not isinstance(self.post, PopulationView) else self.post.population
    )
    if pre_pop != post_pop:
    ...
    self._store_connectivity(
        all_to_all,
        (weights, delays, allow_self_connections, storage_format, storage_order),
        delays,
        storage_format,
        storage_order,
    )
...
def connect_gaussian(
    ...
    self.connector_description = (
        f"Gaussian, amplitude {amp}, sigma {sigma}, delays {_process_random(delays)}"
    )
...
def connect_fixed_probability(
    ...
    self.connector_description = (
        "Random, sparseness %(proba)s, weights %(weight)s, delays %(delay)s"
        % {
            "weight": _process_random(weights),
            "delay": _process_random(delays),
            "proba": probability,
...
def connect_fixed_number_pre(
    ...
    self.connector_description = (
        "Random Convergent %(number)s $\\rightarrow$ 1, weights %(weight)s, delays %(delay)s"
        % {
            "weight": _process_random(weights),
            "delay": _process_random(delays),
            "number": number,
...
def connect_fixed_number_post(
    ...
    self.connector_description = (
        "Random Divergent 1 $\\rightarrow$ %(number)s, weights %(weight)s, delays %(delay)s"
        % {
            "weight": _process_random(weights),
            "delay": _process_random(delays),
            "number": number,
...
def connect_with_func(
    ...
    **args,
    ...
    synapses = method(
        self.pre, self.post, ConfigManager().get("dt", net_id=self.pre.net_id), **args
    )
...
def connect_from_matrix_market(
    self: "Projection",
    filename: str,
    storage_format: str = None,
    storage_order: str = None,
...
def _load_from_lil(
    self: "Projection", pre: "Population", post: "Population", synapses: LILConnectivity
):
    """
...
def connect_from_sparse(
    self,
    weights: "scipy.sparse.lil_matrix",
    delays: int | float = 0.0,
    storage_format: str = None,
    storage_order: str = None,
) -> "Projection":
    """
    ...
    self._store_connectivity(
        self._load_from_sparse, (weights, delays), delays, storage_format, storage_order
    )
...
def connect_from_file(
    ...
    try:
        ...
        if data["delay"] is not None:
            if lil.uniform_delay == -1:
                lil.delay = [
                    list(np.array(tmp) / ConfigManager().get("dt", self.net_id))
                    for tmp in data["delay"]
                ]
            else:
    ...
    self._store_connectivity(
        self._load_from_lil,
        (lil,),
        lil.max_delay if lil.uniform_delay > 0 else lil.delay,
        storage_format=storage_format,
        storage_order=storage_order,
    )
