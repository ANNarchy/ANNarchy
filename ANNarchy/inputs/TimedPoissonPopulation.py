...
class TimedPoissonPopulation(SpecificPopulation):
    ...
    * 5 Hz until the end of the simulation.
    ...
    def __init__(
        self, geometry, rates, schedule, period=-1.0, name=None, copied=False, net_id=0
    ):
        neuron = Neuron(
            parameters="""
            proba = 1.0
        ...
        ):  # One rate for the whole population
    ...
    def __setattr__(self, name, value):
        if name == "schedule":
            ...
            if self.initialized:
                ...
                if isinstance(
                    value[0], (float, int)
                ):  # same value for each neuron, create a list of list
                    value = [
                        [float(value[i]) for _ in range(self.size)]
                        for i in range(len(value))
                    ]
                else:
                    value = [
                        list(np.array(value[i]).flatten()) for i in range(len(value))
                    ]
