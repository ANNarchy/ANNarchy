...
class ImagePopulation(Population):
    ...
    **Warning:** due to the indexing system of Numpy, a 640*480 image should be fed into a (480, 640) or (480, 640, 3) population.
    ...
    def __init__(
        self, geometry: tuple, name: str = None, copied: bool = False, net_id=0
    ):
        # Check geometry
    ...
    def set_image(self, filename: str) -> None:
        ...
        self.r = np.array(im).reshape(self.size) / 255.0
...
class VideoPopulation(ImagePopulation):
    ...
    **Warning:** due to the indexing system of Numpy, a 640*480 image should be fed into a (480, 640) or (480, 640, 3) population.
    ...
    def __init__(
        ...
        # Create the population
        ImagePopulation.__init__(
            self, geometry=geometry, name=name, copied=copied, net_id=net_id
        )
...
"""
    ...
    def start_camera(self, camera_port=0):
        ...
        self.cyInstance.start_camera(
            camera_port,
            self.geometry[1],
            self.geometry[0],
            3 if self.dimension == 3 else 1,
        )
