...
class DiagonalProjection(Projection):
    ...
    def _copy(self, pre, post):
        ...
        return DiagonalProjection(
            pre=pre, post=post, target=self.target, name=self.name, copied=True
        )
    ...
    def connect_gaussian(self, amp, sigma, min_val, max_distance=0.0):
        ...
        for dist_w in range(
            int(self.offset_w) - self.pre.geometry[0] - self.pre.geometry[2],
            int(self.offset_w) + self.post.geometry[2],
        ):
            for dist_h in range(
                int(self.offset_h) - self.pre.geometry[1] - self.pre.geometry[3],
...
"""
        ...
        self._specific_template["psp_code"] = wsum % {
            ...
            "offset": self.offset,
            "dim_post_0": dim_post_0,
            "dim_post_1": dim_post_1,
            "dim_pre_0": dim_pre_0,
            "dim_pre_1": dim_pre_1,
...
""" % {"float_prec": ConfigManager().get("precision", self.net_id)}
        ...
        self._specific_template["psp_code"] = wsum % {
            ...
            "name_post": self.post.name,
            "size_post": self.post.size,
            "predim0": self.pre.geometry[0],
            "predim1": self.pre.geometry[1],
            "predim2": self.pre.geometry[2],
            "predim3": self.pre.geometry[3],
            "postdim0": self.post.geometry[0],
            "postdim1": self.post.geometry[1],
            "postdim2": self.post.geometry[2],
            "postdim3": self.post.geometry[3],
