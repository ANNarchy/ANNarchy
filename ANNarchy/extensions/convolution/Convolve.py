...
class Convolution(SpecificProjection):
    ...
    * If the kernel has more dimensions than the pre-synaptic population, this means a bank of different filters will be applied on the pre-synaptic population (like a convolutional layer in a CNN). Attention: the first index of ``weights`` corresponds to the different filters, while the result will be accessible in the last dimension of the post-synaptic population. You must set the ``multiple`` argument to True. Example:
    ...
    def connect_filter(
        ...
        if (
            self.dim_post < self.dim_pre
        ):  # OK, but check the last dimension of the kernel has the same size as the post-population
    ...
    def _copy(self, pre, post, net_id=None):
        ...
        if self.weights is not None:
            if self._used_single_filter:
                ...
                raise ValueError(
                    "Either use single filter or bank of filter must be True! (Missing connect?)"
                )
            copied_proj._create()
    ...
    def _connect(self, module):
        ...
        if self.delays > 0.0:
            self.cyInstance.set_delay(
                int(self.delays / ConfigManager().get("dt", self.net_id))
            )
    ...
    def _generate_pre_coordinates(self):
        ...
        for dim in range(self.dim_pre):
            if dim < self.dim_post:
                ...
                idx_range.append(
                    [int((sample - 1) / 2) + sample * i for i in range(post_size)]
                )
    ...
    def _generate_pre_coordinates_bank(self):
        ...
        if self.subsampling:
            ...
            self.pre_coordinates = [
                c + [d] for c in self.subsampling for d in range(self.nb_filters)
            ]
            return
    ...
    def _generate(self):
        ...
        if not self.multiple:
            convolve_code, sum_code = self._generate_convolve_code(
                pre_load_inner_line=use_inner_line
            )
        else:
...
"""
        ...
        if kernel:
            ...
            self._specific_template["access_parameters_variables"] = (
                conv_filter_template["openmp"]["access"] % {"type_w": cpp_type_w}
            )
        ...
        target_code = (
            "_sum_%(target)s"
            if self.post.neuron_type.type == "rate"
            else "g_%(target)s"
        )
...
"""
        ...
        if kernel:
            ...
            self._specific_template["export_parameters_variables"] = ""
            self._specific_template["access_parameters_variables"] = (
                conv_filter_template["cuda"]["access"]
                % {"type_w": cpp_type_w, "id_proj": self.id}
            )
        ...
        if not self.multiple:
            ...
            self._specific_template["psp_body"] = (
                cuda_convolution_single_filter["body"] % code_ids
            )
            self._specific_template["psp_invoke"] = (
                cuda_convolution_single_filter["invoke"] % code_ids
            )
            self._specific_template["psp_header"] = (
                cuda_convolution_single_filter["header"] % code_ids
            )
            self._specific_template["psp_call"] = (
                cuda_convolution_single_filter["call"] % code_ids
            )
            ...
            code_ids.update(
                {"filter_dim": self.dim_kernel, "num_elem_filter": num_elem_per_filter}
            )
            ...
            if len(self.weights.shape) == 3 and len(self.pre.geometry) == 2:
                ...
                self._specific_template["psp_body"] = (
                    cuda_convolution_bank_of_filter_3d["body"] % code_ids
                )
                self._specific_template["psp_invoke"] = (
                    cuda_convolution_bank_of_filter_3d["invoke"] % code_ids
                )
                self._specific_template["psp_header"] = (
                    cuda_convolution_bank_of_filter_3d["header"] % code_ids
                )
                self._specific_template["psp_call"] = (
                    cuda_convolution_bank_of_filter_3d["call"] % code_ids
                )
                ...
                self._specific_template["psp_body"] = (
                    cuda_convolution_bank_of_filter_4d["body"] % code_ids
                )
                self._specific_template["psp_invoke"] = (
                    cuda_convolution_bank_of_filter_4d["invoke"] % code_ids
                )
                self._specific_template["psp_header"] = (
                    cuda_convolution_bank_of_filter_4d["header"] % code_ids
                )
                self._specific_template["psp_call"] = (
                    cuda_convolution_bank_of_filter_4d["call"] % code_ids
                )

            else:
                self._specific_template["psp_body"] = (
                    cuda_convolution_bank_of_filter["body"] % code_ids
                )
                self._specific_template["psp_invoke"] = (
                    cuda_convolution_bank_of_filter["invoke"] % code_ids
                )
                self._specific_template["psp_header"] = (
                    cuda_convolution_bank_of_filter["header"] % code_ids
                )
                self._specific_template["psp_call"] = (
                    cuda_convolution_bank_of_filter["call"] % code_ids
                )

        # Post-neuron is a spike neuron (e.g., part of ANN-to-SNN conversion)
        if self.post.neuron_type.type == "spike":
            self._specific_template["psp_call"] = self._specific_template[
                "psp_call"
            ].replace("gpu__sum_" + self.target, "gpu_g_" + self.target)

        # Remove trailing spaces
        self._specific_template["psp_body"] = remove_trailing_spaces(
            self._specific_template["psp_body"]
        )
    ...
    def _generate_convolve_code(self, pre_load_inner_line=True):
        ...
        * pre_load_inner_line: for CPU-code it's useful to have a local variable for accessing the innermost sub-vector
        ...
        if self.keep_last_dimension:
            id_dict = {"index": indices[self.dim_kernel], "dim": self.dim_kernel}
        ...
        if not pre_load_inner_line:
            code += tabify(
                """
                w_idx = %(value)s;"""
                % {"value": self._filter_coordinates_to_index("w", self.weights.shape)},
                dim,
            )
    ...
    """
        ...
        if not pre_load_inner_line:
            code += tabify(
                ...
                w_idx = %(value)s;"""
                % {
                    "value": self._filter_coordinates_to_index(
