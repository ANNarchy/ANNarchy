...
class Pooling(SpecificProjection):
    ...
    def pooling(self, extent: tuple = None, delays: float = 0.0):
        ...
        if extent is None:  # compute the extent automatically
            ...
            Messages.error(
                "Pooling: You must provide a tuple for the extent of the pooling operation."
            )
    ...
    def _connect(self, module):
        ...
        if not self._connection_method:
            Messages.error(
                "Pooling: The projection between "
                + self.pre.name
                + " and "
                + self.post.name
    ...
    def _generate(self):
        ...
        if _check_paradigm("openmp", self.net_id):
            ...
            Messages.error("Pooling: not implemented for the configured paradigm")
...
""" % {"sum_default": sum_default}
        ...
        for dim in range(self.dim_pre):
            ind_dict = {"index": indices[dim], "size": self.extent[dim]}
    ...
    """
        ...
        # Compute indices
        for dim in range(self.dim_pre):
            ind_dict = {"index": indices[dim], "dim": dim}
            if self.extent[dim] > 1:
                code += (
                    """
                int %(index)s_pre = coord[%(dim)s] + %(index)s_w;"""
                    % ind_dict
                )
            else:
                code += (
        ...
        # Check indices
        for dim in range(self.dim_pre):
            ind_dict = {"index": indices[dim], "max_size": self.pre.geometry[dim] - 1}
            code += (
                """
        ...
        if self.delays > ConfigManager().get("dt", self.net_id):
            psp = psp.replace(
                ...
                "pop%(id_pre)s->_delayed_r[%(delay)s][rk_pre]"
                % {
                    "id_pre": self.pre.id,
                    "delay": str(
                        int(self.delays / ConfigManager().get("dt", self.net_id)) - 1
                    ),
                },
            )
        ...
        if operation == "max":
            ...
            Messages.error(
                ...
                operation,
                "is not implemented yet for shared projections with pooling.",
            )
    ...
    def _generate_cuda(self):
        ...
        if len(self.pre.geometry) == 2:
            ...
            if self.extent[0] < 6:
                ...
                pool_template["psp_body"] = (
                    cuda_pooling_code_2d_small_extent["psp_body"] % pool_dict
                )
                pool_template["psp_invoke"] = (
                    cuda_pooling_code_2d_small_extent["psp_invoke"] % pool_dict
                )
                pool_template["psp_header"] = (
                    cuda_pooling_code_2d_small_extent["psp_header"] % pool_dict
                )
                pool_template["psp_call"] = (
                    cuda_pooling_code_2d_small_extent["psp_call"] % pool_dict
                )
                ...
                pool_template["psp_invoke"] = remove_trailing_spaces(
                    cuda_pooling_code_2d["psp_invoke"] % pool_dict
                )
                pool_template["psp_header"] = (
                    cuda_pooling_code_2d["psp_header"] % pool_dict
                )
            ...
            pool_template["psp_invoke"] = remove_trailing_spaces(
                cuda_pooling_code_3d["psp_invoke"] % pool_dict
            )
        ...
        if self.post.neuron_type.type == "spike":
            pool_template["psp_call"] = pool_template["psp_call"].replace(
                "gpu__sum_" + self.target, "gpu_g_" + self.target
            )
