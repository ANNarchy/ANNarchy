...
class NanoBindGenerator:
    ...
    def _generate_pop_wrapper(self, pop: "Population") -> str:
        ...
        # synaptic delay
        attributes += (
            f"""\t\t.def("update_max_delay", &PopStruct{pop.id}::update_max_delay)\n"""
        )
        ...
        for attr in pop.neuron_type.description["attributes"]:
            ...
            if _check_paradigm("cuda", self.net_id):
                ...
                if attr in pop.neuron_type.description["global"] and attr in [
                    tmp["name"] for tmp in pop.neuron_type.description["variables"]
                ]:
                    attributes += """\t\t.def_rw("{name}_host_to_device", &PopStruct{id}::{name}_host_to_device)\n""".format(
                        id=pop.id, name=attr
                    )
        ...
        if pop.neuron_type.type == "spike":
            ...
            if pop.neuron_type.refractory or pop.refractory:
                additional_func += (
                    f"""\t\t.def_rw("refractory", &PopStruct{pop.id}::refractory)\n"""
                )
                if _check_paradigm("cuda", self.net_id):
...
"""
        ...
        datatypes = {"local": [], "semiglobal": [], "global": []}
        ...
        structural_plasticity = ConfigManager().get(
    ...
    def _generate_pop_mon_wrapper(self, pop: "Population") -> str:
        ...
        for attr in pop.neuron_type.description["variables"]:
            ...
            clear_container += (
                """\t\t.def("clear_{name}", &PopRecorder{id}::clear_{name})\n""".format(
                    id=pop.id, name=attr["name"]
                )
            )
    ...
    def _generate_proj_mon_wrapper(self, proj: "Projection") -> str:
        ...
        for attr in proj.synapse_type.description["variables"]:
            record_flag += """\t\t.def_rw("record_{name}", &ProjRecorder{id}::record_{name})\n""".format(
                id=proj.id, name=attr["name"]
            )

            record_container += (
                """\t\t.def_rw("{name}", &ProjRecorder{id}::{name})\n""".format(
                    id=proj.id, name=attr["name"]
                )
            )
    ...
    // Custom global functions"""
        ...
        for _, function in GlobalObjectManager().get_functions():
            # Extract the name
            desc = extract_functions(
                description=function, local_global=True, net_id=self.net_id
            )[0]
            # Wrapper
            wrapper_code += f"""
