...
def generate_non_ODE_block(
    variables, locality, obj, conductance_only, wrap_w, mask_variable, split_loop=False
):
    "TODO: documentation"
    block_code = ""
    block_bounds = ""
    ...
    for param in odes:
        ...
        if not param["name"].startswith("g_"):
            switch = (
                param["switch"]
                if mask_variable is None
                else append_mask_multiplier(param["switch"], mask_variable)
            )
        else:
            switch = param["switch"]
...
%(switch)s
%(bounds)s
""" % {"comment": "// " + param["eq"], "bounds": bounds, "switch": switch}

    return code


def generate_equation_code(
    desc,
    locality="local",
    obj="pop",
...
def determine_idx_type_for_projection(proj):
    ...
    *ANNarchy/generator/Template/PyxTemplate.py*
    ...
    if (
        proj._storage_format != "lil"
        and ConfigManager().get("num_threads", proj.net_id) > 1
    ):
        return "int", "int", "int", "int"
...
def cpp_connector_available(connector_name, desired_format, storage_order, net_id):
    ...
    cpp_patterns = {
        "st": {
            ...
            "pre_to_post": {"csr": ["Random", "Random Convergent"]},
        },
        "omp": {
            "post_to_pre": {"lil": ["Random"], "csr": [], "coo": [], "ell": []},
            "pre_to_post": {"csr": ["Random"]},
        },
        "cuda": {
            "post_to_pre": {
                "csr": ["Random", "Random Convergent"],
...
def check_simd_instructions_type(simd_instr_set="avx", net_id: int = 0):
    ...
    * simd_instr_set: either "sse4_1", "avx" or "avx512f".
    ...
    try:
        ...
        lscpu_txt = (
            subprocess.check_output("lscpu | grep 'Flags' ", shell=True).strip()
        ).decode()
        ...
        try:
            ...
            lscpu_txt = (
                subprocess.check_output(
                    "lscpu | grep 'Markierungen' ", shell=True
                ).strip()
            ).decode()
            return simd_instr_set in lscpu_txt
