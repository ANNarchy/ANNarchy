...
from ANNarchy.generator.Template.GlobalOperationTemplate import (
    global_operation_templates_cuda as global_op_template,
)
    ...
    def header_struct(self, pop, annarchy_dir):
        ...
        if pop.neuron_type.description["type"] == "spike":
            ...
            if pop.neuron_type.refractory or pop.refractory:
                ...
                if isinstance(
                    pop.neuron_type.description["refractory"], str
                ):  # no need to instantiate refractory
                    init_spike += spike_tpl["refractory"]["init_extern"] % {
                        "id": pop.id
                    }
                else:
        ...
        if pop.neuron_type.type == "rate":
            ...
            update_body, update_invoke, update_header, update_call = (
                self._update_spiking_neuron(pop)
            )
        ...
        pop_desc["host_to_device"] = (
            tabify("pop%(id)s->host_to_device();" % {"id": pop.id}, 1) + "\n"
        )
    ...
    // Delayed variables"""
        if pop.neuron_type.type == "rate":
            for var in pop.delayed_variables:
                ...
                declare_code += delay_tpl[attr["locality"]]["declare"] % {
                    "var": var,
                    "type": attr["ctype"],
                }
        else:
...
"""
        ...
        if pop.neuron_type.type == "spike":
            ...
            """ % {
                "max_delay": int(
                    ceil(pop.max_delay / ConfigManager().get("dt", self._net_id))
                )
            }
            update_code += """
        ...
        for func in pop.neuron_type.description["functions"]:
            ...
            if ConfigManager().get("precision", self._net_id) == "float":
                device_code += cpp_func.replace(
                    ...
                    % {"id": pop.id, "func": func["name"]},
                )
            else:
                device_code += cpp_func.replace(
                    "double " + func["name"],
                    "__device__ double pop%(id)s_%(func)s"
                    % {"id": pop.id, "func": func["name"]},
                )
        ...
        for func in pop.neuron_type.description["functions"]:
            ...
            for term in func_occur:
                glob_eqs = loc_eqs.replace(
                    term,
                    term.replace(
                        func["name"], "pop" + str(pop.id) + "_" + func["name"]
                    ),
                )
    ...
    def _replace_random(self, loc_eqs, glob_eqs, random_distributions):
        ...
        for rd in random_distributions:
            if rd["locality"] == "local":
                ...
                if rd["dist"] == "Uniform":
                    term = (
                        """( curand_uniform%(postfix)s( &loc_state_%(rd)s ) * (%(max)s - %(min)s) + %(min)s )"""
                        % {
                    ...
                    term = (
                        """( curand_normal%(postfix)s( &loc_state_%(rd)s ) * %(sigma)s + %(mean)s )"""
                        % {
                            "postfix": prec_extension,
                            "rd": rd["name"],
                            "mean": rd["args"].split(",")[0],
                            "sigma": rd["args"].split(",")[1],
                        }
                    )
                elif rd["dist"] == "LogNormal":
                    term = (
                        """( curand_log_normal%(postfix)s( &loc_state_%(rd)s, %(mean)s, %(std_dev)s) )"""
                        % {
                            "postfix": prec_extension,
                            "rd": rd["name"],
                            "mean": rd["args"].split(",")[0],
                            "std_dev": rd["args"].split(",")[1],
                        }
                    )
                else:
                    Messages._error(
                ...
                if rd["dist"] == "Uniform":
                    term = (
                        """( curand_uniform%(postfix)s( &state_%(rd)s[0] ) * (%(max)s - %(min)s) + %(min)s )"""
                        % {
                            "postfix": prec_extension,
                            "rd": rd["name"],
                            "min": rd["args"].split(",")[0],
                    ...
                    term = (
                        """( curand_normal%(postfix)s( &state_%(rd)s[0] ) * %(sigma)s + %(mean)s )"""
                        % {
                            "postfix": prec_extension,
                            "rd": rd["name"],
                            "mean": rd["args"].split(",")[0],
                    ...
                    term = (
                        """( curand_log_normal%(postfix)s( &state_%(rd)s[0], %(mean)s, %(std_dev)s) )"""
                        % {
                            "postfix": prec_extension,
                            "rd": rd["name"],
                            "mean": rd["args"].split(",")[0],
...
"""
    ...
    def _update_rate_neuron(self, pop):
        ...
        if "update_variables" in pop._specific_template.keys():
            try:
                return (
                    pop._specific_template["update_variable_body"],
                    pop._specific_template["update_variable_invoke"],
                    pop._specific_template["update_variable_header"],
        ...
        # some defaults
        ids = {"id": pop.id, "local_index": "[i]", "global_index": "[0]"}

        # parse the equations
        glob_eqs = generate_equation_code(
            pop.neuron_type.description, locality="global", padding=1
        )
        loc_eqs = generate_equation_code(
            pop.neuron_type.description, locality="local", padding=2
        )

        # Remove %(global_index)s for global parameters
        for par in pop.neuron_type.description["parameters"]:
            if par["locality"] == "global":
                glob_eqs = glob_eqs.replace(
                    par["name"] + "%(global_index)s", par["name"]
                )
...
"""
        ...
        loc_eqs, glob_eqs, rng_pre_loop, rng_post_loop = self._replace_random(
            loc_eqs, glob_eqs, pop.neuron_type.description["random_distributions"]
        )
        ...
        # Global operations
        if glob_eqs.strip() != "":
            add_args_header, add_args_invoke, add_args_call = self._gen_kernel_args(
                pop, "global"
            )
            ...
            device_kernel += CUDATemplates.population_update_kernel["global"][
                "device_kernel"
            ] % {
                "id": pop.id,
            ...
            kernel_invoke += CUDATemplates.population_update_kernel["global"][
                "invoke_kernel"
            ] % {
                "id": pop.id,
                "add_args": add_args_header,
            ...
            kernel_decl += CUDATemplates.population_update_kernel["global"][
                "kernel_decl"
            ] % {"id": pop.id, "add_args": add_args_header}
            global_call = CUDATemplates.population_update_kernel["global"][
                "host_call"
        ...
        if loc_eqs.strip() != "":
            ...
            for target in sorted(
                ...
                add_args_call += """, pop%(id)s->gpu__sum_%(target)s""" % {
                    "id": pop.id,
                    "target": target,
                }
            ...
            # finalize code templates
            device_kernel += CUDATemplates.population_update_kernel["local"][
                "device_kernel"
            ] % {
                "id": pop.id,
                "add_args": add_args_header,
            ...
            kernel_invoke += CUDATemplates.population_update_kernel["local"][
                "invoke_kernel"
            ] % {
                "id": pop.id,
                "add_args": add_args_header,
            ...
            kernel_decl += CUDATemplates.population_update_kernel["local"][
                "kernel_decl"
            ] % {"id": pop.id, "add_args": add_args_header}
            local_call = CUDATemplates.population_update_kernel["local"][
                "host_call"
            ] % {"id": pop.id, "add_args": add_args_call}
...
""" + tabify(pre_code, 1)
        ...
        if pop.neuron_type.refractory or pop.refractory:
            refr_eqs = generate_equation_code(
                pop.neuron_type.description, "local", conductance_only=True, padding=3
            )
...
""" % {"refr_eqs": refr_eqs, "loc_eqs": loc_eqs}
                ...
                add_args_call += """, pop%(id)s->gpu_refractory_remaining""" % {
                    "id": pop.id
                }
            ...
            device_kernel += CUDATemplates.population_update_kernel["local"][
                ...
                "add_args": add_args_header,
                "pop_size": pop.size,
                "pre_loop": tabify(pre_code, 1),
                "post_loop": tabify(rng_post_loop, 1),
                "local_eqs": loc_eqs,
            }
            device_invoke += CUDATemplates.population_update_kernel["local"][
                "invoke_kernel"
            ] % {
                "id": pop.id,
                "add_args": add_args_header,
    ...
    def _spike_gather(self, pop):
        ...
        *host_call*:        device function call
        ...
        if "spike_gather_body" in pop._specific_template.keys():
            try:
                return (
                    pop._specific_template["spike_gather_body"],
                    pop._specific_template["spike_gather_invoke"],
                    pop._specific_template["spike_gather_header"],
...
""" % {"reset": eq["cpp"]}
        ...
        if pop.neuron_type.refractory or pop.refractory:
            ...
            if isinstance(pop.neuron_type.refractory, str):
                found = False
                for param in (
                    pop.neuron_type.description["parameters"]
                    + pop.neuron_type.description["variables"]
                ):
                ...
                header_args += ", %(type)s *%(name)s, int* refractory_remaining" % {
                    "type": param["ctype"],
                    "name": param["name"],
                }
                header_invoke += ", %(name)s, refractory_remaining" % {
                    "name": param["name"]
                }
                call_args += (
                    ", pop%(id)s->gpu_%(name)s, pop%(id)s->gpu_refractory_remaining"
                    % {"id": pop.id, "name": param["name"]}
                )
                ...
                call_args += (
                    """, pop%(id)s->gpu_refractory, pop%(id)s->gpu_refractory_remaining"""
                    % {"id": pop.id}
                )
        else:
            refrac_inc = ""
        ...
        if pop.max_delay > 1:
            default_args = (
                "t, dt, pop%(id)s->gpu_delayed_spiked.front(), pop%(id)s->gpu_last_spike"
                % {"id": pop.id}
            )
        else:  # no_delay
            default_args = "t, dt, pop%(id)s->gpu_spiked, pop%(id)s->gpu_last_spike" % {
                "id": pop.id
            }
    ...
    // host to device transfers for %(name)s""" % {"name": pop.name}
        for attr in pop.neuron_type.description["variables"]:
            ...
            if attr["name"] in pop.neuron_type.description["local"]:
                host_device_transfer += (
                    self._templates["attribute_transfer"]["HtoD_local"] % ids
                )
            else:
                host_device_transfer += (
                    self._templates["attribute_transfer"]["HtoD_global"] % ids
                )
        for attr in pop.neuron_type.description["parameters"]:
            ids = {"id": pop.id, "attr_name": attr["name"], "type": attr["ctype"]}
            if attr["name"] in pop.neuron_type.description["local"]:
...
""" % {"id": pop.id}
        ...
        for attr in pop.neuron_type.description["variables"]:
            ...
            if attr["name"] in pop.neuron_type.description["local"]:
                device_host_transfer += (
                    self._templates["attribute_transfer"]["DtoH_local"] % ids
                )
            else:
                device_host_transfer += (
                    self._templates["attribute_transfer"]["DtoH_global"] % ids
                )
