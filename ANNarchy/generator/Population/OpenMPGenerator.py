...
from ANNarchy.generator.Template.GlobalOperationTemplate import (
    global_operation_templates_omp_extern as global_op_extern_dict,
)
...
class OpenMPGenerator(PopulationGenerator):
    ...
    def header_struct(self, pop, annarchy_dir):
        ...
        if pop.neuron_type.description["type"] == "spike":
            ...
            if pop.neuron_type.refractory or pop.refractory:
                ...
                if isinstance(
                    ...
                    init_spike += spike_specific_tpl["refractory"]["init_extern"] % {
                        "id": pop.id
                    }
                else:
        ...
        if len(pop.global_operations) > 0:
            pop_desc["gops_update"] = (
                """\tpop%(id)s->update_global_ops(tid, nt);\n""" % {"id": pop.id}
            )
        ...
        if pop.neuron_type.type == "rate":
            for var in pop.delayed_variables:
                ...
                if attr["locality"] == "local":
                    declare_code += (
                        """
    std::deque< std::vector< %(type)s > > _delayed_%(name)s; """
                        % attr_dict
                    )
                else:
                    declare_code += (
                        """
    std::deque< %(type)s > _delayed_%(name)s; """
                        % attr_dict
                    )
        else:
...
"""
    ...
    std::deque< %(type)s > _delayed_%(name)s; """
        ...
        for var in pop.delayed_variables:
            attr = self._get_attr(pop, var)
            init_code += delay_tpl[attr["locality"]]["init"] % {
                "name": attr["name"],
                "type": attr["ctype"],
            }
            update_code += delay_tpl[attr["locality"]]["update"] % {"name": var}
            reset_code += delay_tpl[attr["locality"]]["reset"] % {
                "id": pop.id,
                "name": var,
            }
            resize_code += delay_tpl[attr["locality"]]["resize"] % {
                "id": pop.id,
                "name": var,
                "type": attr["ctype"],
            }
...
"""
    ...
    def _update_globalops(self, pop):
        ...
        if True:  # parallel reduction is currently disabled
            from ANNarchy.generator.Template.GlobalOperationTemplate import (
                global_operation_templates_st_call as call_template,
            )
...
%(code)s
            ...
            from ANNarchy.generator.Template.GlobalOperationTemplate import (
                global_operation_templates_omp_call as call_template,
            )
            from ANNarchy.generator.Template.GlobalOperationTemplate import (
                global_operation_templates_omp_reduce as red_template,
            )
...
%(code)s
    ...
    def _update_random_distributions(self, pop):
        ...
        rng_code = (
            self._templates["rng"]["omp_code_seq"]
            if not use_parallel_rng
            else self._templates["rng"]["omp_code_par"]
        )
        ...
        for rd in pop.neuron_type.description["random_distributions"]:
            ...
            if rd["locality"] == "local":
                if not use_parallel_rng:
                    local_code += self._templates["rng"][rd["locality"]]["update"] % {
                        "id": pop.id,
                        "rd_name": rd["name"],
                        "index": 0,
                    }
                ...
                global_code += self._templates["rng"][rd["locality"]]["update"] % {
                    "id": pop.id,
                    "rd_name": rd["name"],
                }
    ...
    def _update_rate_neuron(self, pop):
        ...
        eqs = generate_equation_code(
            pop.neuron_type.description, locality="global", padding=3
        )
        eqs = eqs % id_dict
        if eqs.strip() != "":
            code += """
...
""" % {
                "omp_code": "#pragma omp for simd"
                if not ConfigManager().get("disable_SIMD_Eq", self._net_id)
                else "#pragma omp for",
                "eqs": eqs,
            }
...
""" % {"eqs": eqs}
        ...
        has_refractory = (
            "in_ref" if (pop.neuron_type.refractory or pop.refractory) else None
        )
...
""" % {
    ...
    def _spike_gather(self, pop):
        ...
        if pop.neuron_type.axon_spike:
            ...
            axon_cond = (
                pop.neuron_type.description["axon_spike"]["spike_cond"] % id_dict
            )
...
""" % {
        ...
        # Is there a refractory period?
        has_refractory = (
            True if (pop.neuron_type.refractory or pop.refractory) else False
        )
...
""" % {"reset": eq["cpp"] % id_dict}
        ...
        # Gather code
        omp_critical_code = (
            "#pragma omp critical" if pop.size > Global.OMP_MIN_NB_NEURONS else ""
        )
        refrac_check = tabify(
            """
