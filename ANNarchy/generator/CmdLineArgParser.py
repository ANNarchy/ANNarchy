...
class CmdLineArgParser(object):
    ...
    def setup_parser(self):
        ...
        self.parser = argparse.ArgumentParser(
            description="ANNarchy: Artificial Neural Networks architect."
        )
        ...
        group.add_argument(
            "--prec",
            help="Set the floating precision used.",
            action="store",
            type=str,
            default=None,
            dest="precision",
        ...
        group.add_argument(
            "--report",
            help="Create a network overview using either .tex or .md.",
            action="store",
            type=str,
            default=None,
            dest="report",
        ...
        group.add_argument(
            "--trace",
            help="Trace initialization of objects (init), function calls during simulation (simulate), or all (both).",
            action="store",
            default=None,
            dest="trace_calls",
        )
        group.add_argument(
            "-d",
            "--debug",
            help="Compilation with debug symbols and additional checks.",
            action="store_true",
            default=False,
            dest="debug",
        )
        ...
        group.add_argument(
            "--auto-tuning",
            help="Enable automatic sparse matrix format selection.",
            action="store_true",
            default=False,
            dest="auto_tuning",
        )
        ...
        group.add_argument(
            "--visible-cores",
            help="Cores where the threads should be placed.",
            type=str,
            action="store",
            default=None,
            dest="visible_cores",
        ...
        group.add_argument(
            "--gpu",
            help="Enables CUDA and optionally specifies the GPU id (default: 0).",
            type=int,
            action="store",
            nargs="?",
            default=-1,
        ...
        group.add_argument(
            "--profile-out",
            help="Target folder for profiling data.",
            action="store",
            type=str,
            default=None,
            dest="profile_out",
    ...
    def parse_arguments_for_setup(self):
        ...
        if options.auto_tuning:
            ...
            ConfigManagement.setup(
                sparse_matrix_format="auto", sparse_matrix_storage_order="auto"
            )
        ...
        if options.trace_calls is not None:
            if options.trace_calls in ["init", "simulate", "both"]:
                ConfigManagement._update_global_config(
                    "trace_calls", options.trace_calls
                )
            else:
                raise ValueError(
                    "--trace accepts only one of the following keys: init, simulate, both"
                )
