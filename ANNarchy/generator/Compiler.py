...
seq:
\tcython -%(major)s build/net%(net_id)s/ANNarchyCore%(net_id)s.pyx --cplus
\tclang++ -stdlib=libc++ -std=c++11 -fPIC -shared %(cpu_flags)s -fpermissive -I. -I%(python_include)s -I%(numpy_include)s build/net%(net_id)s/*.cpp  -L/Users/vitay/anaconda/lib -L%(python_lib)s -lpython%(py_version)s -ldl -framework CoreFoundation %(libs)s -o ANNarchyCore%(net_id)s.so

clean:
\trm -rf build/net%(net_id)s/*.o
\trm -rf build/net%(net_id)s/*.so
""" % {'cpu_flags': cpu_flags, 'libs': libs, 'py_version': py_version, 'major': py_major, 'python_include': get_python_inc(), 'python_lib': get_python_lib(), 'numpy_include': numpy_include, 'net_id': self.net_id}
