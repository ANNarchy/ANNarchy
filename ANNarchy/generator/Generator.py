...
\tg++ -march=native %(cpu_flags)s -shared -fPIC -fpermissive -std=c++0x -I. -I/usr/include/python%(py_version)s -I %(numpy_include)s build/*.cpp %(libs)s -o ANNarchyCore.so

omp:
\tcython build/ANNarchyCore.pyx --cplus
\tg++ -march=native %(cpu_flags)s -shared -fPIC -fpermissive -std=c++0x -I. -I/usr/include/python%(py_version)s -I %(numpy_include)s -fopenmp build/*.cpp %(libs)s -o ANNarchyCore.so
...
seq:
\tcython build/ANNarchyCore.pyx --cplus
\tclang++ -stdlib=libc++ -std=c++11 -fPIC -shared %(cpu_flags)s -fpermissive -I. -I/usr/include/python%(py_version)s -I %(numpy_include)s build/*.cpp -lpython %(libs)s -o ANNarchyCore.so
