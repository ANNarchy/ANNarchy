...
\tg++ -march=native %(cpu_flags)s -shared -fPIC -fpermissive -std=c++0x -I. -I/usr/include/python%(py_version)s -I %(numpy_include)s %(libs)s build/*.cpp -o ANNarchyCore.so

omp:
\tcython build/ANNarchyCore.pyx --cplus
\tg++ -march=native %(cpu_flags)s -shared -fPIC -fpermissive -std=c++0x -I. -I/usr/include/python%(py_version)s -I %(numpy_include)s -fopenmp %(libs)s build/*.cpp -o ANNarchyCore.so
