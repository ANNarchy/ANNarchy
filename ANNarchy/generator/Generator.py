...
all:
\t@echo "Use either openmp or cuda"
...
cuda:
\tcython build/ANNarchyCore.pyx --cplus
\tnvcc -genvcc -gencode arch=compute_20,code=compute_20 -c kernel.cu -o kernel.o
\tg++ -march=native -O2 -shared -fPIC -fpermissive -std=c++0x -I. -I/usr/include/python2.7 -fopenmp build/*.cpp -o ANNarchyCore.so
...
"""
            ...
            try:
                if Global.config['paradigm'] == "openmp":
                    ...
                    subprocess.check_output("make cuda -j4 "+ pipe_to_cmd_line, 
                                            shell=True)
