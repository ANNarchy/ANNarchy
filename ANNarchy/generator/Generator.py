...
seq:
\tcython -%(major)s build/ANNarchyCore.pyx --cplus
\tg++ -march=native %(cpu_flags)s -shared -fPIC -fpermissive -std=c++0x -I. -I/usr/include/python%(py_version)s -I/usr/include/python%(py_version)sm -I %(numpy_include)s build/*.cpp %(libs)s -o ANNarchyCore.so

omp:
\tcython -%(major)s build/ANNarchyCore.pyx --cplus
\tg++ -march=native %(cpu_flags)s -shared -fPIC -fpermissive -std=c++0x -I. -I/usr/include/python%(py_version)s -I/usr/include/python%(py_version)sm -I %(numpy_include)s -fopenmp build/*.cpp %(libs)s -o ANNarchyCore.so
...
\trm -rf build/*.o
\trm -rf build/*.so
""" % {'cpu_flags': cpu_flags, 'gpu_flags': gpu_flags, 'libs': libs, 'py_version': py_version, 'numpy_include': numpy_include, 'major': py_major}

        elif sys.platform == "darwin":   # mac os
            src = """# Makefile generated by ANNarchy
seq:
\tcython -%(major)s build/ANNarchyCore.pyx --cplus
\tclang++ -stdlib=libc++ -std=c++11 -fPIC -shared %(cpu_flags)s -fpermissive -I. -I/usr/include/python%(py_version)s -I/usr/include/python%(py_version)sm -I %(numpy_include)s build/*.cpp -lpython %(libs)s -o ANNarchyCore.so

clean:
\trm -rf build/*.o
\trm -rf build/*.so
""" % {'cpu_flags': cpu_flags, 'libs': libs, 'py_version': py_version, 'numpy_include': numpy_include}
