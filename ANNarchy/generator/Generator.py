...
\tg++ -march=native %(flags)s -shared -fPIC -fpermissive -std=c++0x -I. -I/usr/include/python%(py_version)s -I %(numpy_include)s %(libs)s build/*.cpp -o ANNarchyCore.so
...
""" % {'flags': flags, 'libs': libs, 'py_version': py_version, 'numpy_include': numpy_include}
            ...
            try:
                if Global.config['num_threads'] > 1:
                    subprocess.check_output("make omp -j4 > compile_stdout.log 2> compile_stderr.log", 
                                            shell=True)
                else:
                    subprocess.check_output("make seq -j4 > compile_stdout.log 2> compile_stderr.log", 
                                            shell=True)
