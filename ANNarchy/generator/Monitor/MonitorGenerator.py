...
class MonitorGenerator(object):
    ...
    def generate(self):
        ...
        with open(
            self._annarchy_dir + "/generate/net" + str(self.net_id) + "/Monitor.hpp",
            "w",
        ) as ofile:
            ofile.write(code)
    ...
    def _pop_recorder_class(self, pop):
        ...
        for var in pop.neuron_type.description["variables"]:
            ...
            size_in_bytes_estimate_code += (
                template[var["locality"]]["size_in_bytes_estimate"] % ids
            )
        ...
        if pop.neuron_type.type == "spike":
            ...
            recording_code += (
                base_tpl["record"][ConfigManager().get("paradigm", self.net_id)]
                % rec_dict
            )
            size_in_bytes_code += (
                base_tpl["size_in_bytes"][ConfigManager().get("paradigm", self.net_id)]
                % rec_dict
            )
    ...
    def _proj_recorder_class(self, proj):
        ...
        for var in proj.synapse_type.description["variables"]:
            ...
            ids = {"type": var["ctype"], "name": var["name"]}
            ...
            size_in_bytes_estimate_code += tabify(
                template[locality]["size_in_bytes_estimate"] % ids, 2
            )
