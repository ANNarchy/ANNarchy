...
from ANNarchy.generator.Template import OpenMPBaseTemplate
from ANNarchy.generator.Template.GlobalOperationTemplate import global_operation_templates_openmp

class OpenMPGenerator(SimCoreGenerator):
    """
    """
...
void set_%(name)s(%(float_prec)s value);"""
    ...
    def generate_body(self, annarchy_dir, source_dest, prof_gen, pop_gen, pop_desc, proj_desc):
        ...
        * cuda: ANNarchyHost.cu and ANNarchyDevice.cu
        ...
        body_code = OpenMPBaseTemplate.body_template % base_dict
...
void set_%(name)s(%(float_prec)s value){%(name)s = value;};"""
    ...
    def _body_def_glops(self):
        ...
        for op in sorted(list(set(ops))):
            code += global_operation_templates_openmp[op] % type_def
    ...
    def _body_run_until(self):
        ...
        for pop in NetworkManager().get_network(self._net_id).get_populations():
            ...
            return OpenMPBaseTemplate.run_until_template["default"]

        # a condition has been defined, so we generate corresponding code
        cond_code = ""
        for pop in NetworkManager().get_network(self._net_id).get_populations():
            if pop.stop_condition:
                cond_code += OpenMPBaseTemplate.run_until_template["single_pop"] % {"id": pop.id}

        return OpenMPBaseTemplate.run_until_template["body"] % {"run_until": cond_code}
