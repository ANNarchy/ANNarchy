...
def check_experimental_features(populations, projections):
    ...
    for proj in projections:
        detected_formats.append(
            (proj._storage_format, proj._storage_order, proj.synapse_type.type)
        )
    ...
    for proj in projections:
        ...
        if proj.synapse_type.type == "spike" and proj._storage_format in [
            ...
            raise Messages.InvalidConfiguration(
                ...
                + "' is not allowed for spiking synapses."
            )
        ...
        if proj.synapse_type.type == "spike":
            if (
                "psp" in proj.synapse_type.description.keys()
                and proj._storage_order == "pre_to_post"
            ):
                raise Messages.InvalidConfiguration(
                    "Using continuous transmission within a spiking synapse prevents the application of pre-to-post matrix ordering"
                )
            ...
            if _check_paradigm("openmp", proj.net_id):
                if proj._storage_format in [
                    ...
                    raise Messages.InvalidConfiguration(
                        "Using 'storage_format="
                        + proj._storage_format
                        + "' is only allowed for default rate-coded synapses yet."
                    )
        ...
        if proj._has_single_weight() and proj._storage_format in ["dense", "bsr"]:
            raise Messages.InvalidConfiguration(
                "Using 'storage_format="
                + proj._storage_format
                + "' is not allowed for single weight projections."
            )
        ...
        if proj._storage_format == "dia":
            if _check_paradigm("cuda", proj.net_id):
                raise Messages.InvalidConfiguration(
                    "Using diagonal format is limited to CPUs yet."
                )

            if proj.pre.size < proj.post.size:
                raise Messages.InvalidConfiguration(
                    "Using diagonal format is not implemented for projections where the pre-synaptic layer is smaller than the post-synaptic one."
                )

            if isinstance(proj.post, PopulationView):
                raise Messages.InvalidConfiguration(
                    "Using diagonal format and post-synaptic PopulationViews is not available."
                )
...
def _check_locality(populations, projections):
    ...
    for proj in projections:
        for var in proj.synapse_type.description["variables"]:
            if (
                var["locality"] == "global"
            ):  # cannot depend on local or semiglobal variables
                # Inside the equation
                for v in var["dependencies"]:
                    if _get_locality(v, proj.synapse_type.description) in [
                        "local",
                        "semiglobal",
                    ]:
            ...
            if (
                var["locality"] == "semiglobal"
            ):  # cannot depend on pre-synaptic variables
                # Inside the equation
                for v in var["dependencies"]:
    ...
    for proj in projections:
        if proj.synapse_type.creating or proj.synapse_type.pruning:
            if _check_paradigm("cuda", proj.net_id):
                ...
                raise Messages.InvalidConfiguration(
                    "Structural plasticity is not supported on GPU devices."
                )
