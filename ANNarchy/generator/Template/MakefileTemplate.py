...
all:
\t%(cython)s -%(py_major)s --cplus %(cython_ext)s -D ANNarchyCore%(net_id)s.pyx
\t%(gpu_compiler)s %(gpu_flags)s -std=c++14 -c -Xcompiler -std=c++14,-fPIC, ANNarchyKernel.cu -o ANNarchyKernel.o
\t%(gpu_compiler)s %(cuda_gen)s %(gpu_flags)s -Xcompiler %(cpu_flags)s-std=c++14,-fPIC,-shared \\
        ANNarchy.cu *.cpp ANNarchyKernel.o -o ANNarchyCore%(net_id)s.so \\
