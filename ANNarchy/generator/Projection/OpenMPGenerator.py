...
"""
        ...
        proj_desc["update"] = (
            ""
            if update_variables == ""
            else """\tproj%(id)s->update_synapse(tid, nt);\n""" % {"id": proj.id}
        )
        ...
        proj_desc["post_event"] = (
            ""
            if post_event == ""
            else """\tproj%(id)s->post_event(tid, nt);\n""" % {"id": proj.id}
        )
        ...
        if proj._storage_format == "lil":
            ...
            if proj.synapse_type.type == "rate":
                ...
                raise Messages.InvalidConfiguration(
                    "    "
                    + proj.name
                    + ": ELLPACK-R format is not available for spiking models."
                )
            ...
            if proj.synapse_type.type == "rate":
                ...
                raise Messages.InvalidConfiguration(
                    "    "
                    + proj.name
                    + ": sliced ELLPACK format is not available for spiking models."
                )
            ...
            if proj.synapse_type.type == "rate":
                ...
                raise Messages.InvalidConfiguration(
                    "    "
                    + proj.name
                    + ": ELLPACK format is not available for spiking models."
                )
    ...
    def creating(self, proj):
        ...
        creation_ids.update({"pre_index": "[rk_pre]", "post_index": "[rk_post]"})
    ...
    def _computesum_rate(self, proj, single_matrix):
        ...
        * single_matrix:    Is the basic connectivity a complete matrix or sliced
            ...
            if simd_type is not None:
                ...
                if proj._storage_format == "bsr":
                    try:
                        ...
                        unrolled_template = template = self._templates[
        ...
        schedule = (
            ""
            if not "psp_schedule" in proj._omp_config.keys()
            else proj._omp_config["psp_schedule"]
        )
        ...
        ids.update(
            {
                ...
                "psp": psp.replace(";", ""),
                "simd_len": str(4)
                if _check_precision("double", self._net_id)
                else str(8),
            }
        )
        ...
        if proj._storage_order == "post_to_pre":
            ...
            if not proj.disable_omp:  # TODO: are there other conditions?
                ...
                for target in targets:
                    psp_prefix += """
        std::vector< double > pop%(id)s_%(target)s_thr(pop%(id)s->get_size()*global_num_threads, 0.0);""" % {
                        "id": proj.post.id,
                        "target": target,
                    }
                psp_prefix += """
        ...
        for eq in proj.synapse_type.description["pre_spike"]:
            ...
            if eq["name"] == "g_target":
                ...
                for target in targets:
                    ...
                    if proj._has_single_weight():
                        g_target = re.sub(
                            r"([^\w]+)w%\(local_index\)s", r"\1w", g_target
                        )
                    ...
                    if (
                        proj.max_delay > 1 and proj.uniform_delay == -1
                    ):  # TODO: openMP is switched off for non uniform delays
                        g_target_code += (
                            """
...
"""
                        ...
                        proj.disable_omp
                        or ConfigManager().get("num_threads", self._net_id) == 1
                    ):
                        g_target_code += (
                            """
            %(post_prefix)sg_%(target)s%(post_index)s %(operation)s %(g_target)s
...
""" % {
                ...
                if (
                    ...
                    simultaneous = (
                        "%(pre_prefix)slast_spike[pre_rank[i][j]] != %(post_prefix)slast_spike[post_rank[i]]"
...
"""
        ...
        if template is None:
            raise Messages.CodeGeneratorException(
                "\tproj{}: no template available (Configuration: format={}, order={}, single_matrix={}, pattern={})".format(
                    proj.id,
                    proj._storage_format,
                    proj._storage_order,
                    single_matrix,
...
""" % {"pre_prefix": ids["pre_prefix"], "pre_array": pre_array}
    ...
    def _header_structural_plasticity(self, proj):
        ...
        if proj.max_delay > 1 and proj.uniform_delay == -1:
            delay_code = (
                " " * 8 + "delay[post].insert(delay[post_idx].begin() + pos, _delay);"
            )
            delay_remove = " " * 8 + "delay[post].erase(delay[post_idx].begin() + pos);"
...
"""
        ...
        try:
            ids.update({"post_event": post_code, "event_driven": event_driven_code})
