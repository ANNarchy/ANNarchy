...
class CUDAGenerator(ProjectionGenerator):
    ...
    def header_struct(self, proj, annarchy_dir):
        ...
        (
            ...
            sparse_matrix_args,
            single_matrix,
        ) = self._select_sparse_matrix_format(proj, suppress_printouts=False)
        ...
        init_weights, init_delays, init_parameters_variables = (
            self._init_parameters_variables(proj, single_matrix)
        )

        variables_body, variables_invoke, variables_header, variables_call = (
            self._update_synapse(proj)
        )
        ...
        # Post event
        post_event_body, post_event_invoke, post_event_header, post_event_call = (
            self._post_event(proj)
        )

        # Compute sum is the trickiest part
        psp_device_kernel, psp_invoke_kernel, psp_kernel_decl, psp_host_call = (
            self._computesum_rate(proj)
            if proj.synapse_type.type == "rate"
            else self._computesum_spiking(proj)
        )
        ...
        if "declare_connectivity_matrix" not in proj._specific_template.keys():
            connector_call = self._connectivity_init(
                proj, sparse_matrix_format, sparse_matrix_args
            ) % {
            ...
            connector_call = ""
            declare_connectivity_matrix = proj._specific_template[
                "declare_connectivity_matrix"
            ]
            access_connectivity_matrix = proj._specific_template[
                "access_connectivity_matrix"
            ]
        ...
        if self._prof_gen:
            ...
            declare_profile, init_profile = self._prof_gen.generate_init_projection(
                proj
            )
        else:
            include_profile = ""
    ...
    def _configure_template_ids(self, proj):
        ...
        if proj._storage_format == "csr":
            ...
            raise Messages.InvalidConfiguration(
                "   The storage_format="
                + str(proj._storage_format)
                + " is not available on CUDA devices"
            )
    ...
    def _generate_launch_config(self, proj):
        ...
        init_code = self._templates["launch_config"]["init"] % {"id_proj": proj.id}
        update_code = self._templates["launch_config"]["update"] % {"id_proj": proj.id}
        return init_code, update_code

    def creating(self, proj):
        raise Messages.CodeGeneratorException(
            "Structural plasticity is not supported on CUDA devices."
        )
    ...
    def _computesum_rate(self, proj):
        ...
        # Specific projection
        if (
            "psp_header" in proj._specific_template.keys()
            or "psp_body" in proj._specific_template.keys()
            or "psp_call" in proj._specific_template.keys()
        ):
            try:
        ...
        if not "psp" in proj.synapse_type.description.keys():  # default
            ...
            for dep in list(set(proj.synapse_type.description["dependencies"]["pre"])):
                ...
                add_args_header += (
                    ", const %(type)s* __restrict__ pre_%(name)s" % attr_ids
                )
        ...
        if proj._has_single_weight():
            psp = re.sub(r"([^\w]+)w%\(local_index\)s", r"\1w", " " + psp)
        ...
        if proj._storage_format != "hyb":
            ...
            device_kernel = (
                self._templates["rate_psp"]["device_kernel"][operation] % body_dict
            )
            ...
            host_call_dict.update(
                {
                    "conn_args": conn_call,
                    "target_arg": ", pop%(id_post)s->gpu__sum_%(target)s"
                    % host_call_dict,
            ...
            for dep in proj.synapse_type.description["psp"]["dependencies"]:
                add_args_call_coo = add_args_call_coo.replace(
                    "gpu_" + dep + ",", "gpu_" + dep + "->coo,"
                )
                add_args_call_ell = add_args_call_ell.replace(
                    "gpu_" + dep + ",", "gpu_" + dep + "->ell,"
                )
    ...
    def _computesum_spiking(self, proj):
        ...
        if proj._storage_format == "csr":
            if proj._storage_order == "post_to_pre":
                ...
                ids.update(
                    {"local_index": "[syn_idx]", "pre_index": "[col_idx[syn_idx]]"}
                )
            ...
            raise Messages.CodeGeneratorException(
                "\tno indices defined for spiking psp_template and storage_format = "
        ...
        for var in proj.synapse_type.description["pre_spike"]:
            if var["name"] == "g_target":  # synaptic transmission
                ...
                for target in sorted(list(set(target_list))):
                    ...
                    if ids["atomicOp"] == "atomicExch" and _check_precision(
                        "double", self._net_id
                    ):
                        # HD (12th April 2023): atomicExch is not defined for double, so we need to use the long long type-cast version
                        psp_code += (
                            "atomicExch((unsigned long long int*)&g_%(target)s%(post_index)s, __double_as_longlong(tmp));\n"
                            % ids
                        )
                    else:
                        psp_code += (
                            "%(atomicOp)s(&g_%(target)s%(post_index)s, tmp);\n" % ids
                        )

                    # Boundary code is optional
                    bound_code = get_bounds(var)
                    if len(bound_code) != 0:
                        bound_code = bound_code.replace(
                            "g_target%(local_index)s", "g_" + target + "%(post_index)s"
                        )
                ...
                if "unless_post" in var["flags"]:
                    simultaneous = (
                        "pop%(id_pre)s_last_spike[_pr] != pop%(id_post)s_last_spike[%(semiglobal_index)s]"
                        % ids
                    )
                    if condition == "":
...
"""
        ...
        pre_post_deps = list(
            set(
                proj.synapse_type.description["dependencies"]["pre"]
        ...
        for dep in kernel_deps:
            ...
            if attr_type == "par" and attr_dict["locality"] == "global":
                ...
                if event_driven_code.strip() != "":
                    event_driven_code = event_driven_code.replace(
                        attr_dict["name"] + "%(global_index)s", attr_dict["name"]
                    )
                if pre_spike_code.strip() != "":
                    pre_spike_code = pre_spike_code.replace(
                        attr_dict["name"] + "%(global_index)s", attr_dict["name"]
                    )
            else:
        ...
        if len(pre_spike_code) == 0 and len(psp_code) == 0:
            ...
            if proj._storage_format == "csr":
                if proj._storage_order == "post_to_pre":
                    conn_args_header = (
                        "%(size_type)s* col_ptr, %(idx_type)s* row_idx, %(idx_type)s* inv_idx, %(float_prec)s *w"
                        % ids
                    )
                    ...
                    conn_call = (
                        "proj%(id_proj)s->gpu_row_ptr, proj%(id_proj)s->gpu_col_idx, proj%(id_proj)s->gpu_w"
                        % ids
                    )
            elif proj._storage_format == "dense":
                conn_args_header = (
                    "const %(idx_type)s row_size, const %(idx_type)s column_size, %(float_prec)s *w"
                    % ids
                )
                conn_args_invoke = "row_size, column_size, w"
                conn_call = (
                    "proj%(id_proj)s->num_rows(), proj%(id_proj)s->num_columns(), proj%(id_proj)s->gpu_w"
                    % ids
                )
            else:
                raise NotImplementedError
            ...
            pre_size = (
                proj.pre.size
                if isinstance(proj.pre, Population)
                else proj.pre.population.size
            )
            post_size = (
                proj.post.size
                if isinstance(proj.post, Population)
                else proj.post.population.size
            )
            ...
            if proj.max_delay > 1:
                if proj.uniform_delay == -1:  # Non-uniform delays
                    ...
                    pre_spike_events = (
                        "pop%(id_pre)s->gpu_delayed_spiked[proj%(id_proj)s->delay-1]"
                        % {"id_pre": proj.pre.id, "id_proj": proj.id}
                    )
                    pre_spike_count = (
                        "pop%(id_pre)s->host_delayed_num_events[proj%(id_proj)s->delay-1]"
                        % {"id_pre": proj.pre.id, "id_proj": proj.id}
                    )
            else:
            ...
            invoke_kernel = template["invoke_kernel"] % {
                ...
                "pre_spike_events": pre_spike_events.replace(
                    "pop" + str(proj.pre.id) + "->gpu_", ""
                ),
                "pre_spike_count": pre_spike_count.replace(
                    "pop" + str(proj.pre.id) + ".", ""
                ),
            }
    ...
    def _declaration_accessors(self, proj, single_matrix):
        ...
        """
        declaration, accessor = ProjectionGenerator._declaration_accessors(
            self, proj, single_matrix
        )
    ...
    def _gen_kernel_args(proj, pop_deps, deps):
        ...
        * deps: list of all attribute names which are dependencies
        ...
        for dep in deps:
            ...
            if dep in pop_deps:
                if dep in proj.synapse_type.description["dependencies"]["pre"]:
                    attr_type, attr_dict = PopulationGenerator._get_attr_and_type(
                        proj.pre, dep
                    )
                    ids = {
                        "type": attr_dict["ctype"],
        ...
        if proj.synapse_type.type == "spike":
            kernel_args_decl = (
                ", long int* pre_last_spike, long int* post_last_spike"
                + kernel_args_decl
            )
            kernel_args_invoke = (
                ", pre_last_spike, post_last_spike" + kernel_args_invoke
            )
            kernel_args_call = (
                ", pop%(id_pre)s->gpu_last_spike, pop%(id_post)s->gpu_last_spike"
    ...
    def _header_structural_plasticity(self, proj):
        raise Messages.CodeGeneratorException(
            "Structural plasticity is not supported on GPUs."
        )
        ...
        for func in proj.synapse_type.description["functions"]:
            ...
            if _check_precision("float", self._net_id):
                device_code += cpp_func.replace(
                    "float" + func["name"],
                    "__device__ float proj%(id)s_%(func)s"
                    % {"id": proj.id, "func": func["name"]},
                )
            else:
                device_code += cpp_func.replace(
                    "double " + func["name"],
                    "__device__ double proj%(id)s_%(func)s"
                    % {"id": proj.id, "func": func["name"]},
                )
    ...
    def _replace_local_funcs(self, proj, glob_eqs, semiglobal_eqs, loc_eqs):
        ...
        for func in proj.synapse_type.description["functions"]:
            ...
            for term in func_occur:
                glob_eqs = loc_eqs.replace(
                    term,
                    term.replace(
                        func["name"], "proj" + str(proj.id) + "_" + func["name"]
                    ),
                )

            func_occur = re.findall(search_term, semiglobal_eqs)
            for term in func_occur:
                semiglobal_eqs = loc_eqs.replace(
                    term,
                    term.replace(
                        func["name"], "proj" + str(proj.id) + "_" + func["name"]
                    ),
        ...
        for dist in random_distributions:
            ...
            if dist["dist"] == "Uniform":
                ...
                if dist["locality"] == "local":
                    term = (
                        """( curand_uniform%(postfix)s( &state_%(rd)s%(local_index)s ) * (%(max)s - %(min)s) + %(min)s )"""
                        % dist_ids
                    )
                ...
                if dist["locality"] == "local":
                    term = (
                        """( curand_normal%(postfix)s( &state_%(rd)s[j] ) * %(sigma)s + %(mean)s )"""
                        % dist_ids
                    )
                    loc_pre += "%(prec)s %(name)s = %(term)s;" % {
                        "prec": ConfigManager().get("precision", self._net_id),
                ...
                if dist["locality"] == "local":
                    term = (
                        """( curand_log_normal%(postfix)s( &state_%(rd)s[j], %(mean)s, %(std_dev)s) )"""
                        % dist_ids
                    )
                    loc_pre += "%(prec)s %(name)s = %(term)s;" % {
                        "prec": ConfigManager().get("precision", self._net_id),
...
""" % {"local_index": "[j]"}
        ...
        kernel_deps = sorted(
            list(set(post_deps + event_deps))
        )  # variables can occur in several eqs
        for dep in kernel_deps:
            if dep == "w":
                continue
    ...
    def _memory_transfers(self, proj):
        ...
        """
        if (
            "host_device_transfer" in proj._specific_template.keys()
            and "device_host_transfer" in proj._specific_template.keys()
        ):
            return proj._specific_template[
                "host_device_transfer"
            ], proj._specific_template["device_host_transfer"]
        ...
        proc_attr = []
        for attr in (
            proj.synapse_type.description["parameters"]
            + proj.synapse_type.description["variables"]
        ):
            # avoid doublons
    ...
    def _process_equations(self, proj, equations, ids, locality):
        ...
        kernel_args, kernel_args_invoke, kernel_args_call = self._gen_kernel_args(
            proj, neur_deps, syn_deps
        )
        ...
        if locality == "semiglobal":
            ...
            if proj._storage_format in ["csr"]:
                if proj._storage_order == "post_to_pre":
                    ...
                    rk_assign += (
                        "%(idx_type)s rk_post = rank_post%(semiglobal_index)s;\n"
                    )
            elif proj._storage_format in ["dense"]:
                pass
            else:
    ...
    def _update_synapse(self, proj):
        ...
        * a tuple contain three strings ( device_kernel, call, header )
        """
        # Global variables
        global_eq = generate_equation_code(
            proj.synapse_type.description,
            "global",
            "proj",
            padding=1,
            wrap_w="plasticity",
        ...
        semiglobal_eq = generate_equation_code(
            proj.synapse_type.description,
            "semiglobal",
            "proj",
            padding=2,
        ...
        local_eq = generate_equation_code(
            proj.synapse_type.description,
            "local",
            "proj",
            padding=pad,
        ...
        if global_eq.strip() != "":
            (
                ...
                global_pre_code,
                kernel_args_global,
                kernel_args_invoke_global,
                kernel_args_call_global,
        ...
        if semiglobal_eq.strip() != "":
            (
                semiglobal_eq,
                semiglobal_pre_code,
                kernel_args_semiglobal,
                kernel_args_invoke_semiglobal,
                kernel_args_call_semiglobal,
        ...
        if local_eq.strip() != "":
            (
                ...
                local_pre_code,
                kernel_args_local,
                kernel_args_invoke_local,
                kernel_args_call_local,
            ) = self._process_equations(proj, local_eq, ids, "local")

        # replace the random distributions
        local_eq, global_eq = self._replace_random(
            local_eq,
            ids["local_index"],
            global_eq,
            proj.synapse_type.description["random_distributions"],
        )

        #
        # replace local function calls
        if len(proj.synapse_type.description["functions"]) > 0:
            global_eq, semiglobal_eq, local_eq = self._replace_local_funcs(
                proj, global_eq, semiglobal_eq, local_eq
            )
        ...
        if global_eq.strip() != "":
            ...
            device_kernel += (
                self._templates["synapse_update"]["global"]["device_kernel"] % body_dict
            )
            ...
            invoke_kernel += (
                self._templates["synapse_update"]["global"]["invoke_kernel"]
                % invoke_dict
            )
            ...
            kernel_header += (
                self._templates["synapse_update"]["global"]["kernel_decl"] % header_dict
            )
            ...
            call_dict.update(
                {
                    ...
                    if isinstance(proj.target, list)
                    else proj.target,
                    "kernel_args_call": kernel_args_call_global,
                }
            )
            global_call = (
                self._templates["synapse_update"]["global"]["host_call"] % call_dict
            )
        ...
        if semiglobal_eq.strip() != "":
            ...
            body_dict.update(ids)
            device_kernel += (
                self._templates["synapse_update"]["semiglobal"]["device_kernel"]
                % body_dict
            )
            ...
            invoke_dict.update(ids)
            invoke_kernel += (
                self._templates["synapse_update"]["semiglobal"]["invoke_kernel"]
                % invoke_dict
            )
            ...
            header_dict.update(ids)
            kernel_header += (
                self._templates["synapse_update"]["semiglobal"]["kernel_decl"]
                % header_dict
            )
            ...
            semiglobal_call = (
                self._templates["synapse_update"]["semiglobal"]["host_call"] % call_dict
            )
        ...
        if local_eq.strip() != "":
            ...
            body_dict.update(ids)
            device_kernel += (
                self._templates["synapse_update"]["local"]["device_kernel"] % body_dict
            )
            ...
            invoke_dict.update(ids)
            invoke_kernel += (
                self._templates["synapse_update"]["local"]["invoke_kernel"]
                % invoke_dict
            )

            header_dict = {"conn_args": conn_header, "kernel_args": kernel_args_local}
            header_dict.update(ids)
            kernel_header += (
                self._templates["synapse_update"]["local"]["kernel_decl"] % header_dict
            )
            ...
            local_call = (
                self._templates["synapse_update"]["local"]["host_call"] % call_dict
            )
