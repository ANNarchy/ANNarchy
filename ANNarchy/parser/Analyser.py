...
class Analyser(object):
                    ...
                    for newvar in new_odes:
                        for oldvar in pop.description['variables']:
                            if newvar['name'] == oldvar['name']:
                                oldvar['cpp'] = newvar['cpp']
                                oldvar['switch'] = newvar['switch']

            self.analysed_populations[name] = pop.description 
            
        # Generate C++ code for all projection variables 
        for name, proj in self.projections.iteritems():

            self.parse_projection(proj)
            self.analysed_projections[name] = proj.description 
    ...
    def parse_population(self, pop):
        ...
        for t in  pop.neuron.description['targets']:
        ...
        for variable in pop.neuron.description['parameters']:
        ...
        for variable in pop.neuron.description['variables']:
            if isinstance(pop.init[variable['name']], bool) or \
               isinstance(pop.init[variable['name']], int) or \
               isinstance(pop.init[variable['name']], float) :
                variable['init'] = pop.init[variable['name']]
        ...
        pop.neuron.description['random_distributions'] = extract_randomdist(pop)

        # Extract the spike condition if any
        if 'raw_spike' in pop.neuron.description.keys():
            pop.neuron.description['spike'] = extract_spike_variable(pop.neuron.description)

        # Extract the stop condition if any
        if pop.neuron.description.has_key('stop_condition'):
        ...
        for variable in pop.neuron.description['variables']:
            ...
            for target in pop.neuron.description['targets']:
                if pop.neuron.description['type'] == 'rate': 
            ...
            for name, val in untouched_globs.iteritems():
                if not untouched.has_key(name):
                    untouched[name] = val
            pop.neuron.description['global_operations'] += global_ops
            ...
            if 'min' in variable['bounds'].keys():
                if isinstance(variable['bounds']['min'], str):
                    translator = Equation(variable['name'], variable['bounds']['min'], 
                                          pop.neuron.description['attributes'], 
                                          pop.neuron.description['local'], 
                                          pop.neuron.description['global'], 
                                          type = 'return',
