    ...
    for variable in description["variables"]:
        ...
        for dist in available_distributions:
            ...
            for l, v in matches:
                ...
                definition = (
                    distributions_equivalents[dist] + "(" + processed_arguments + ")"
                )
...
def extract_prepost(name, eq, description, net_id):
    ...
    for var in list(set(pre_matches)):
        if var == "sum":  # pre.sum(exc)
            ...
            eq = re.sub(
                r"pre." + var + r"([^_\w]+)", "_pre_" + var + r"__\g<1>", eq + " "
            )
    ...
    for var in list(set(post_matches)):
        if var == "sum":  # post.sum(exc)
            ...
            eq = re.sub(
                r"post." + var + r"([^_\w]+)", r"_post_" + var + r"__\g<1>", eq + " "
            )
            # eq = eq.replace(target, ' _post_'+var+'__')
            untouched["_post_" + var + "__"] = (
                "%(post_prefix)s" + var + "%(post_index)s"
            )
    ...
    if isinstance(description, (dict,)):
        ...
        for definition in description:
            ...
            bounds, flags, ctype, init = extract_boundsflags(
                constraint, equation, extra_values, net_id=net_id
            )
...
def extract_functions(description, net_id, local_global=False):
    ...
    for f in function_list:
        ...
        if condition == []:
            ...
            parsed_content, deps = translate_ITE(
                "", eq2, condition, arguments, {}, function=True
            )
            arguments = list(
                set(arguments)
            )  # somehow the entries in arguments are doubled ... ( HD, 23.02.2017 )
        ...
        if not local_global:  # local to a class
            oneliner = (
                """%(return_type)s %(name)s (%(arg_line)s) {return %(parsed_content)s ;};
"""
                % fdict
            )
        else:  # global
            oneliner = (
                """inline %(return_type)s %(name)s (%(arg_line)s) {return %(parsed_content)s ;};
"""
                % fdict
            )
        fdict["cpp"] = oneliner
...
def extract_structural_plasticity(statement, description, net_id=0):
    ...
    eq, untouched, dependencies = extract_prepost(
        "test", eq, description, net_id=net_id
    )
