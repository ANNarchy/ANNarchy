...
def prepare_string(stream):
    ...
    for expr in tmp_set:
        ...
        if (
            expr.strip() == "" or len(expr) == 0
        ):  # through beginning line breaks or something similar empty strings are contained in the set
            continue
...
def extract_name(equation, left=False):
    ...
    # Check for error
    if name.strip() == "":
        Messages.error("the variable name can not be extracted from " + equation)
    ...
    * 'constraints' is all the constraints given after the last :. _extract_flags() should be called on it.
    ...
    for line in equations:
        ...
        if len(split_operators) == 1:  # definition of a new variable
            ...
            if eq.strip() == "":
                print(equation)
                Messages.error("The equation can not be analysed, check the syntax.")

            name = extract_name(eq, left=True)
            if name in ["_undefined", ""]:
                Messages.error("No variable name can be found in " + equation)

            # Append the result
            variables.append(
                {"name": name, "eq": equation.strip(), "constraint": constraint.strip()}
            )
            ...
            Messages.error("Only one assignement operator is allowed per equation.")
