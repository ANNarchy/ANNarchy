...
class CoupledEquations(Equation):
    ...
    def solve_implicit(self, expression_list):
        ...
        for name, expression in expression_list.items():
            for n in self.names:
                expression = re.sub(
                    r"([^\w]+)" + n + r"([^\w]+)", r"\1_" + n + r"\2", expression
                )
            expression = expression.replace(
                "_t_gradient_", "(_" + name + " - " + name + ")"
            )
            expression_list[name] = expression
        ...
        for var, sol in solution.items():
            ...
            switch = (
                sp.ccode(self.local_dict[new_vars[var]]) + " = _" + new_vars[var] + ";"
            )

            # Replace untouched variables with their original name
            for prev, new in sorted(
                list(self.untouched.items()), key=lambda key: len(key[0]), reverse=True
            ):
    ...
    def solve_midpoint(self, expression_list):
        ...
        for name, val in self.local_dict.items():
            tmp_dict[name] = val
        for name, evaluation in evaluations.items():
            tmp_dict[name] = sp.Symbol(
                "("
                + sp.ccode(self.local_dict[name])
                + " + 0.5 * dt * _k_"
                + name
                + " )"
            )
        ...
        for name, expression in expression_list.items():
            switches[name] = (
                sp.ccode(self.local_dict[name]) + " += dt * _" + name + " ;"
            )
    ...
    def solve_rk4(self, expression_list):
        ...
        for name, expression in expression_list.items():
            tmp_analysed = self.parse_expression(expression, local_dict=tmp_dict_k2)
        ...
        for name, expression in expression_list.items():
            tmp_analysed = self.parse_expression(expression, local_dict=tmp_dict_k3)
        ...
        for name, evaluation in evaluations.items():
            tmp_dict_k4[name] = sp.Symbol(
                "(" + sp.ccode(self.local_dict[name]) + " + dt*_k3_" + name + " )"
            )

        # Compute the values _k4_x = f(x + dt*_k3_x)
        for name, expression in expression_list.items():
            tmp_analysed = self.parse_expression(expression, local_dict=tmp_dict_k4)
