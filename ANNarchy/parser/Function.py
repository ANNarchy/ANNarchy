...
class FunctionParser(object):
    ...
    def __init__(
        ...
        * untouched: list of terms which should not be modified
        ...
        for i in range(10):
            self.local_dict["__conditional__" + str(i)] = sp.Symbol(
                "__conditional__" + str(i)
            )
    ...
    def parse(self, part=None):
        ...
        try:
            c_code = sp.ccode(
                eq, precision=8, strict=False, user_functions=self.user_functions
            )
        except TypeError:
            c_code = sp.ccode(eq, precision=8, user_functions=self.user_functions)
        return c_code
