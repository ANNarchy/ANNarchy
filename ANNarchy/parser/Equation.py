...
class Equation(object):
    ...
    def identify_type(self):
        ...
        * simple for the rest, e.g. "r = pos(mp)" or "baseline = Uniform(0,1)"
        ...
        if (
            len(split_expr) == 1
        ):  # no equal sign = treat as condition (return statements will be specified
            return "cond"
    ...
    def c_code(self, equation):
        ...
        try:
            c_code = sp.ccode(
                equation, precision=8, strict=False, user_functions=self.user_functions
            )
        except TypeError:
    ...
    def analyse_ODE(self, expression):
        ...
        if self.method == "semiimplicit":
            ...
            raise NotImplementedError(
                "The chosen numerical method ", self.method, "is not available"
            )
    ...
    def midpoint(self, expression):
        ...
        tmp_dict[self.name] = sp.Symbol(
            "(" + self.c_code(variable_name) + " + 0.5*_k_" + self.name + " )"
        )
        tmp_analysed = self.parse_expression(expression, local_dict=self.local_dict)
    ...
    def runge_kutta_4(self, expression):
        ...
        tmp_dict[self.name] = sp.Symbol(
            "(" + self.c_code(variable_name) + " + 0.5 * dt * _k1_" + self.name + " )"
        )
        tmp_analysed = self.parse_expression(expression, local_dict=tmp_dict)
        tmp_equation_k2 = sp.solve(tmp_analysed, new_var, check=False, rational=False)[
            0
        ]
        explicit_code += (
            ConfigManager().get("precision", self.net_id)
        ...
        tmp_dict = deepcopy(self.local_dict)
        tmp_dict[self.name] = sp.Symbol(
            "(" + self.c_code(variable_name) + " + 0.5 * dt * _k2_" + self.name + " )"
        )
        tmp_analysed = self.parse_expression(expression, local_dict=tmp_dict)
        tmp_equation_k3 = sp.solve(tmp_analysed, new_var, check=False, rational=False)[
            0
        ]
        explicit_code += (
            ConfigManager().get("precision", self.net_id)
        ...
        tmp_dict = deepcopy(self.local_dict)
        tmp_dict[self.name] = sp.Symbol(
            "(" + self.c_code(variable_name) + " + dt * _k3_" + self.name + " )"
        )
        tmp_analysed = self.parse_expression(expression, local_dict=tmp_dict)
        tmp_equation_k4 = sp.solve(tmp_analysed, new_var, check=False, rational=False)[
            0
        ]
        explicit_code += (
            ConfigManager().get("precision", self.net_id)
            ...
            + self.name
            + " = ("
            + self.c_code(tmp_equation_k4)
            + ");\n"
        )

        # final x is part of k1 .. k4
        switch = (
            self.c_code(variable_name)
            + " += dt/6.0 * ( _k1_"
            + self.name
            + " + (_k2_"
            + self.name
    ...
    def implicit(self, expression):
        ...
        # Parse the string
        analysed = self.parse_expression(new_expression, local_dict=self.local_dict)
        self.analysed = analysed
    ...
    def exponential(self, expression):
        ...
        for var in self.attributes:  # Add each variable of the neuron
            if (
                var in self.local_attributes
                and self.local_dict[var] in stepsize.atoms()
            ):
    ...
    def eventdriven(self, expression):
        ...
        if steady == "0":
            code = (
                variable_name
                + " *= exp(dt*(_last_event%(local_index)s - (t))/("
                + self.c_code(real_tau)
                + "));"
            )
        else:
            code = (
    ...
    def standardize_ODE(self, expression):
        ...
        expression = expression.replace(
            "d" + self.name + "/dt", "_gradvar_"
        )  # TODO: robust to spaces
        ...
        collected_var = sp.collect(
            expanded, self.local_dict[self.name], evaluate=False, exact=False
        )
        ...
        collected_gradient = sp.collect(
            sp.expand(analysed, grad_var), grad_var, evaluate=False, exact=False
        )
        ...
        # Steady state A
        steadystate = sp.simplify(
            sp.together(real_tau * grad_var + self.local_dict[self.name] - normalized)
        )
    ...
    def analyse_condition(self, expression):
        ...
        if "==" in expression:
            ...
            if "&" in expression or "|" in expression:
                expression = re.sub(
                    r"([\w\s.]+)==([\w\s.]+)", r"Equality(\1, \2)", expression
                )
            else:  # only one term
                # Remove brackets
        ...
        if "!=" in expression:
            ...
            if "&" in expression or "|" in expression:
                expression = re.sub(
                    r"([\w\s.]+)!=([\w\s.]+)", r"Not(Equality(\1, \2))", expression
                )
            else:  # only one term
                # Remove brackets
                if expression[0] == "(" and expression[-1] == ")":
    ...
    def analyse_increment(self, expression):
        ...
        # Obtain C code
        code = (
            self.c_code(self.local_dict[name])
            + ope
            + self.c_code(sp.simplify(analysed, ratio=1.0))
            + ";"
        )
    ...
    def analyse_assignment(self, expression):
        ...
        # Parse the string
        self.analysed = self.parse_expression(expression, local_dict=self.local_dict)

        # Obtain C code
        code = (
            self.c_code(self.local_dict[name])
            + " = "
            + self.c_code(self.analysed)
            + ";"
        )
