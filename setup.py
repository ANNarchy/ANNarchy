...
def create_config():
    ...
    def cuda_config():
        ...
        return {"compiler": "nvcc", "flags": "", "device": 0, "path": cuda_path}
                ...
                for key in settings[paradigm].keys():
                    if not key in local_settings[paradigm].keys():
                        ...
                        if local_settings[paradigm][key] != settings[paradigm][key]:
                            ...
                            print(
                                "       - your annarchy.json: ",
                                local_settings[paradigm][key],
                            )
                            print(
                                "       - ANNarchy default: ", settings[paradigm][key]
                            )
...
def python_environment():
    ...
    with subprocess.Popen(
        cmd % {"py_version": py_version, "py_prefix": py_prefix}, shell=True
    ) as test:
        if test.wait() != 0:
            print(
    ...
    with subprocess.Popen(
        py_prefix
        + "/bin/cython%(major)s -V > /dev/null 2> /dev/null" % {"major": py_major},
        shell=True,
    ) as test:
        if test.wait() != 0:
    ...
    with subprocess.Popen(
        ...
        if test.wait() != 0:
            ...
            if cython is None:
                ...
                if cython is None:
                    print(
                        "ERROR: Unable to find the 'cython' executable, fix your $PATH if already installed."
                    )
                    sys.exit(1)
...
extensions = [
    ...
    Extension(
        "ANNarchy.cython_ext.Coordinates",
    ...
    Extension(
        "ANNarchy.cython_ext.Transformations",
...
class CustomizedBuild(build):
    ...
    def run(self):
        ...
        py_version, _, python_include, python_libpath, cython_major = (
            python_environment()
        )
