...
class Matrix8x6_WithAllRows: public ::testing::Test {
protected:
    ...
    void SetUp() override {
        ...
        pre_ranks_.push_back(std::vector<int>({5}));
        ...
        if constexpr(std::is_same_v<MatrixType, BSRMatrix<int, int, char, true>> ||
            ...
            // SELL needs slice size S, here I choose 4 this means two blocks will be created.
            mat_ = new MatrixType(8, 6, 4);
        } else {
    ...
    StorageType generate_inited_matrix_variable() {
        ...
        if constexpr(std::is_same_v<MatrixType, BSRMatrix<int, int, char, true>> ||
            ...
            std::cout << "Matrix8x6_WithAllRows::generate_inited_matrix_variable: Non-implemented case ..." << std::endl;
        }
    ...
    StorageType generate_expected_matrix_variable(int pattern) {
        ...
        if constexpr(std::is_same_v<MatrixType, BSRMatrix<int, int, char, true>> ||
            ...
            std::cout << "Matrix8x6_WithAllRows::generate_expected_matrix_variable: non-implemented case ..." << std::endl;
        }
...
TYPED_TEST_SUITE(Matrix8x6_WithAllRows, MatrixImplementations);
...
TYPED_TEST(Matrix8x6_WithAllRows, DenseSizeAccessors) {
    // dense sizes   
    EXPECT_EQ(this->mat_->num_rows(), 8);
    EXPECT_EQ(this->mat_->num_columns(), 6);
}

TYPED_TEST(Matrix8x6_WithAllRows, PostRanksAccessor) {
    // read-out
...
TYPED_TEST(Matrix8x6_WithAllRows, PreRanksAccessor) {
...
TYPED_TEST(Matrix8x6_WithAllRows, GetNumberOfDendrites) {
    // Compare number of rows with non-zeros
    EXPECT_EQ(this->mat_->nb_dendrites(), this->pre_ranks_.size());
}

TYPED_TEST(Matrix8x6_WithAllRows, GetNumberOfSynapses) {
    // How many non-zeros
    int nnz = 0;
...
TYPED_TEST(Matrix8x6_WithAllRows, GetDendriteSize) {
...
TYPED_TEST(Matrix8x6_WithAllRows, GetDendriteRanks) {
...
/*
TYPED_TEST(Matrix8x6_WithAllRows, InitMatrixWithOneValue) {
    // Reference
...
TYPED_TEST(Matrix8x6_WithAllRows, InitMatrixWithUniform) {
    std::mt19937 rng;
...
/*
TYPED_TEST(Matrix8x6_WithAllRows, UpdateMatrixSinglePosition) {
...
TYPED_TEST(Matrix8x6_WithAllRows, UpdateMatrixSingleRow) {
    // Reference
...
TYPED_TEST(Matrix8x6_WithAllRows, UpdateMatrixAllAtOnce) {
    // Init variable
    auto matrix_variable = this->mat_->template init_matrix_variable<double>(1.0);
...
/*
TYPED_TEST(Matrix8x6_WithAllRows, GetMatrixSinglePosition) {
...
TYPED_TEST(Matrix8x6_WithAllRows, GetMatrixSingleRow) {
...
TYPED_TEST(Matrix8x6_WithAllRows, GetMatrixAllAtOnce) {
...
/*
TYPED_TEST(Matrix8x6_WithAllRows, InitVectorWithOneValue) {
