...
class Matrix5x8_WithAllRows: public ::testing::Test {
protected:
    ...
    void SetUp() override {
        ...
        pre_ranks_.push_back(std::vector<int>({3, 6}));
        ...
        pre_ranks_.push_back(std::vector<int>({3, 4, 5}));
        ...
        mat_ = new PartitionedMatrix<PartMatrixType, int, int>(5, 8);

        // This will results in two partitions: 3x8 and 2x8
        mat_->init_matrix_from_lil(post_ranks_, pre_ranks_, 2);
    }
...
TYPED_TEST_SUITE(Matrix5x8_WithAllRows, PartitionedMatrixImplementations);
...
TYPED_TEST(Matrix5x8_WithAllRows, DenseSizeAccessors) {
    // dense sizes   
    EXPECT_EQ(this->mat_->num_rows(), 5);
    EXPECT_EQ(this->mat_->num_columns(), 8);
}

TYPED_TEST(Matrix5x8_WithAllRows, PostRanksAccessor) {
    // read-out
...
TYPED_TEST(Matrix5x8_WithAllRows, PreRanksAccessor) {
...
TYPED_TEST(Matrix5x8_WithAllRows, GetNumberOfDendrites) {
    // Compare number of rows with non-zeros
    EXPECT_EQ(this->mat_->nb_dendrites(), this->pre_ranks_.size());
}

TYPED_TEST(Matrix5x8_WithAllRows, GetNumberOfSynapses) {
    // How many non-zeros
    int nnz = 0;
...
TYPED_TEST(Matrix5x8_WithAllRows, GetDendriteSize) {
...
TYPED_TEST(Matrix5x8_WithAllRows, GetDendriteRanks) {
